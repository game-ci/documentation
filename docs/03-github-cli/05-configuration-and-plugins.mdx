...
### Not a plugin: crash-symbol collection

Debug symbols have to be captured at build time or they are gone for good - once the build machine
is torn down, every future crash report from that build is unsymbolicatable. That makes symbol
collection an output-collection concern rather than new command surface, so it lives in the
Orchestrator as the `symbols` output type, alongside the built-in `coverage`/`logs`/`metrics` types
- not as a plugin. It finds dSYM bundles, PDB, Breakpad and IL2CPP maps under a build; uploading is
handled by the Orchestrator's existing artifact upload path.
