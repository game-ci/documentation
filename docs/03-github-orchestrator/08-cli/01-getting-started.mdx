...
| Provider-backed jobs through a friendly CLI | `game-ci orchestrate` from the public GameCI CLI |
| GitHub Actions Unity builds                 | `game-ci/orchestrator@v1.0.0`                    |
...
The `game-ci/orchestrator` action wraps Unity Builder and exposes Orchestrator's provider and service
inputs. You do not need to install the standalone CLI when using the action.
