...
This split is a deliberate architectural line, not just a feature gap: core `game-ci`
`build`/`test`/`activate` stay thin, engine-invocation wrappers on purpose, so they stay easy to
read and safe to depend on. Anything beyond invoking the engine once — caching, retry/recovery,
extensibility hooks, wrapping the engine's own process launch — is Orchestrator's job. See the
CLI's own [Orchestrate: advanced topics](/docs/cli/orchestrate-advanced) for the `game-ci
orchestrate`-side surface (local caching, middleware, build retry, the engine launch wrapper).
