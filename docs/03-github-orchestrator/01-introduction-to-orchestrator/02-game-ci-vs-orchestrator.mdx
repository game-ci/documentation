...
Standard GameCI is the minimal, container-focused foundation. It exposes the essential Unity build
features clearly and keeps the CI workflow close to the platform you already use.

Orchestrator builds on that foundation when a project needs advanced automation: richer caching,
warm workspaces, provider abstraction, self-hosted runner coordination, load balancing, custom
hooks, async execution, and game-specific performance features.
...
| Standard GameCI on self-hosted | One owned runner is enough and you can manage its state yourself.                 |
| Orchestrator with local Docker | You want Orchestrator behavior locally before cloud or farm rollout.              |
| Orchestrator with self-hosting | Owned runners need locks, fallback, cleanup, warm workspaces, or shared cache.    |
...
- Unity editor containers maintained by GameCI
- License activation and return behavior
...
Orchestrator is for teams whose build pipeline has become an infrastructure problem.
...
A self-hosted runner alone gives you owned hardware. Orchestrator adds build-farm behavior on top of
owned hardware: workspace locking, cache services, fallback routing, cleanup, hooks, and provider
parity with cloud builds.
...
imports, long builds, scarce hardware, multiple providers, and teams that need consistent behavior
across all of them.
