...
on-premise, local Docker, or your local machine. In GitHub Actions, `game-ci/orchestrator` wraps
Unity Builder and adds provider-backed execution. The public `game-ci` CLI includes built-in engine
and Orchestrator plugins, and its plugin API can add more.
...
Use the `game-ci/orchestrator` action when a workflow needs `providerStrategy` or another
Orchestrator input. It invokes Unity Builder as part of the action; no separate builder step is
needed.
...
For command-line builds, use the public [GameCI CLI](/docs/cli). Orchestrator is built in.
