...
| Method                    | Description                                                                                                               |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **GitHub Action `with`**  | Pass parameters directly in your workflow file. See [Getting Started](introduction-to-orchestrator/getting-started).      |
| **Command Line**          | Pass parameters as CLI flags. See [Command Line](introduction-to-orchestrator/examples/command-line).                     |
...
| `garbage-collect`       | Clean up old resources. See [Garbage Collection](advanced-topics/build-reliability#garbage-collection). |
...
| ---------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `providerStrategy`     | `local`                 | Provider type to use. Built-in: `aws`, `k8s`, `local-docker`, `local`. Also accepts `config:<path>` for [config-defined providers](providers/config-defined-providers), plus GitHub URLs, NPM packages, or local paths for [custom providers](providers/custom-providers). |
...
| `enginePlugin` | -       | Engine provider plugin source for custom or overridden engine behavior. NPM package, `cli:<path>`, or `docker:<image>`. See [Engine Plugins](advanced-topics/engine-plugins). |
...
| `customJob`               | -       | Custom job YAML to override the default build workflow. See [Custom Jobs](advanced-topics/build-services#custom-jobs). |
...
| `maxRetainedWorkspaces` | `0`             | Maximum number of [retained workspaces](advanced-topics/caching#retained-workspaces). `0` = unlimited. Above the limit, jobs use standard caching. |
