...
[Game CI unity-builder](../docker/01-getting-started.mdx).** It dispatches Unity builds to cloud
infrastructure, self-hosted runners, or local Docker containers instead of running them directly on
your CI runner. Start jobs from GitHub Actions, the command line, or any CI system. Orchestrator
provisions the environment, syncs your project, runs the build, and streams results back.
...
The orchestrator is available as a standalone package
[`@game-ci/orchestrator`](https://github.com/game-ci/orchestrator). When installed, unity-builder
automatically detects and loads it via the plugin interface. It can also be used independently from
the command line.
