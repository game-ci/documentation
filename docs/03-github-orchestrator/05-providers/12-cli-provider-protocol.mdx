...
If your provider only needs to run fixed lifecycle commands from YAML or JSON, start with
[Config-Defined Providers](config-defined-providers). Use the CLI provider protocol when an
executable needs to parse structured requests, maintain more control over streaming output, or wrap
logic that is too complex for a configuration file.

```mermaid
graph LR
    Orch["providerExecutable<br/>./my-provider<br/>Orchestrator spawns your<br/>executable per subcommand"]
    Exec["Your executable<br/>setup-workflow<br/>run-task<br/>cleanup-workflow<br/>garbage-collect<br/>list-resources<br/>list-workflow<br/>watch-workflow"]
...
## Config vs CLI vs TypeScript Providers

| Feature       | Config-defined provider | CLI provider          | TypeScript provider     |
| ------------- | ----------------------- | --------------------- | ----------------------- |
| Language      | YAML or JSON            | Any                   | TypeScript/JavaScript   |
| Setup         | `providerStrategy`      | `providerExecutable`  | `providerStrategy`      |
| Communication | Environment variables   | JSON stdin/stdout     | Direct function calls   |
| Streaming     | Command stdout/stderr   | stdout lines          | Native logging          |
| Distribution  | File in your repo       | Any executable        | GitHub, NPM, local path |
| Dependencies  | Shell and called tools  | None (self-contained) | Node.js runtime         |

Use [config-defined providers](config-defined-providers) when shell commands are enough. Use CLI
providers when you want to write in a non-TypeScript language, need to wrap an existing tool, or
want a self-contained executable with no Node.js dependency.
...
- [Config-Defined Providers](config-defined-providers) - YAML and JSON provider definitions
