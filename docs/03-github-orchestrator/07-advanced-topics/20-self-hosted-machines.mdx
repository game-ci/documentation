...
Self-hosted machines can make game CI much faster, but persistent workspaces also accumulate state
across builds. Aborted jobs, disk errors, concurrent engine instances, cross-runner cache restores,
and oversized local caches introduce optimization and reliability problems that ephemeral runners
never encounter.

This page covers patterns for keeping persistent build machines fast, predictable, and recoverable
across thousands of CI runs. The examples are Unity-heavy because Unity `Library` caching is the
most common high-impact case, but the same principles apply to Godot import caches, Unreal
intermediate data, custom engine providers, and lab machines running multiple projects.
