...
This page also covers operational reliability features that keep provider runs observable and clean:
log streaming, stale resource cleanup, and automatic garbage collection.

## Logging

Orchestrator streams provider logs back to the CI runner so the dispatcher job can show build
progress without waiting for post-processing.

```mermaid
flowchart LR
  A["Provider job\nBuild output"] --> B["Orchestrator\nLog stream"] --> C["CI runner\nConsole output"]
```

Provider transports differ:

| Provider   | Log transport                                               |
| ---------- | ----------------------------------------------------------- |
| Kubernetes | Native Kubernetes pod logs.                                 |
| AWS        | CloudWatch Logs with a Kinesis subscription for live reads. |
| Local      | Direct process or Docker output on the host.                |

Enable verbose diagnostics with `orchestratorDebug: true` when investigating provider setup,
resource allocation, environment variables, disk usage, or cache behavior.
...
Orchestrator cleans up provider resources after normal builds. If a dispatcher job is cancelled, a
provider API call fails, or a process exits before cleanup runs, stale resources can remain. Garbage
collection removes those old resources by age.

```mermaid
flowchart LR
  A["Create resources"] --> B["Build"]
  B --> C["Normal cleanup"]
  B -. interrupted .-> D["Stale resources"]
  D --> E["garbage-collect\nremoves after garbageMaxAge"]
```

Run garbage collection from GitHub Actions:
...
`garbageMaxAge` is measured in hours. For AWS, Orchestrator can also create a CloudFormation-based
cleanup cron job when `useCleanupCron` is enabled.
...
processes can hold Library file handles, compete with the next Unity launch, or leave satellite
processes running after the parent editor exits.
...
This option is intended for persistent Windows hosts. It is usually unnecessary for fresh Linux
containers where the operating system process table is discarded after each job.
...
Recovery decisions use independent retry budgets per failure type. A licensing retry, for example,
does not consume the budget for PackageCache cleanup or crash recovery.

For the broader failure-handling model across providers, caches, runner startup, engine execution,
artifacts, and cleanup, see [Failures and Diagnostics](failures-and-diagnostics).

### Selective Library Cleanup

Full Library deletion is expensive and should be a last resort. The reliability service exposes
smaller cleanup operations for targeted recovery:
...
When Library caches are shared across self-hosted runners with different workspace paths, several
metadata files contain absolute paths that become stale. Orchestrator uses a **repair-over-delete**
approach: instead of deleting cached data and forcing Unity to regenerate it (which can take
10-15 minutes for a full Bee recompile), stale paths are replaced in-place with the current workspace
path.

### Files Containing Absolute Paths

| Location | Files | Content | Approach |
| --- | --- | --- | --- |
| `Library/Bee/` | `*.dag`, `*.dag.json`, `*.dag.outputdata`, `*-inputdata.json` | Build graph metadata with workspace paths | Sed-replace old path with current |
| `Library/Bee/artifacts/` | Compiled DLLs | Runner-independent compiled output | **Preserved** (no path repair needed) |
| `Library/PackageManager/` | `projectResolution.json`, `ProjectCache` | Package resolution with absolute paths | Sed-replace old path with current |
| `Library/PackageCache/` | Versioned package directories | Content-addressed by fingerprint hash | **Preserved** (no paths to repair) |
| `Library/ScriptAssemblies/` | Compiled script DLLs | Runner-independent compiled output | **Preserved** (cleared only on profile change) |

### How Path Repair Works

The cache restore service (`LocalCacheService`) runs two repair passes after restoring a Library
cache:

1. **`repairDagFiles()`** scans `Library/Bee/` for DAG and companion files, finds absolute paths
   that do not match the current workspace, and replaces them. If a file cannot be read or repaired,
   it is deleted as a fallback (Unity rebuilds DAG files on next compilation).

2. **`repairPackageManagerPaths()`** repairs `projectResolution.json` and `ProjectCache` in
   `Library/PackageManager/`. These files map package names to absolute directory paths. Stale paths
   cause "Could not restore immutable package asset" errors that cascade into thousands of CS0246
   compilation failures. If repair fails, the files are deleted (Unity regenerates them from
   `manifest.json` and `packages-lock.json` in about 5-10 seconds).

### Why Not Delete?

Deleting `Library/Bee/artifacts/` forces a full Bee recompile that takes 10-15 minutes on large
projects. The artifacts themselves are compiled managed DLLs that work regardless of which runner
compiled them. Only the DAG metadata files reference the workspace path. By repairing the metadata
and preserving the artifacts, cross-runner builds get the full benefit of cached compilation.
