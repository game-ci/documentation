...
Preflight tests catch many common failures before expensive builds run — see
...
When a build exceeds a configured time limit, Orchestrator can force garbage collection regardless
of the `constantGarbageCollection` setting. This prevents resource leaks from long-running or stuck
builds.

```yaml
- uses: game-ci/unity-builder@v4
  with:
    gcTimeoutMinutes: 120 # Force GC if build exceeds 2 hours
    garbageMaxAge: 24
```

Set `gcTimeoutMinutes` to `0` (default) to disable timeout-based GC.

### Max Cache Entries

By default, Orchestrator keeps the 2 most recent cache snapshots per folder. Use `maxCacheEntries`
to adjust how many tar archives are retained:

```yaml
- uses: game-ci/unity-builder@v4
  with:
    maxCacheEntries: 5 # Keep 5 most recent cache snapshots per folder
```

Lower values save disk space; higher values allow rolling back to older cache states.

### Min Cache Entries

When `cacheRetentionDays` is set, age-based GC removes old cache directories. Use `minCacheEntries`
to set a floor — the GC will never remove entries below this count, even if they exceed the age
threshold:
...
- uses: game-ci/unity-builder@v4
  with:
    ...
    minCacheEntries: 1 # Always keep at least 1 cache entry per key
```

This prevents the cache from being completely emptied when all entries are older than the retention
period.
