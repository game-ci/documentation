...
Self-hosted runners with persistent workspaces accumulate state across builds. Aborted jobs, disk
errors, concurrent Unity instances, and cross-runner cache restores introduce failure modes that
ephemeral runners never encounter. This page covers patterns for keeping persistent Windows runners
healthy across thousands of CI runs.

These patterns apply to any self-hosted runner setup — bare metal, VMs, or on-prem machines — where
the workspace survives between jobs.
