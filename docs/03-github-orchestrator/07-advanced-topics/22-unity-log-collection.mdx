...
Add `collectUnityLogs: true` to your `game-ci/orchestrator` step:
...
These inputs are accepted by `game-ci/orchestrator`. They also work as
`--collectUnityLogs`-style flags on the standalone Orchestrator CLI.
