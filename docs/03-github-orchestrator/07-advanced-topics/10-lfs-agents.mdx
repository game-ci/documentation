...
Orchestrator supports custom Git LFS transfer agents — external executables that handle
LFS upload and download instead of the default HTTPS transport.
...
The `lfsStoragePaths` input sets the `LFS_STORAGE_PATHS` environment variable. How these paths
are interpreted depends on the agent:
