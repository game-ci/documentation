...
Traditional providers follow a cold-start model: provision → clone → cache restore → build →
tear down. Hot runners eliminate startup latency by keeping editors warm and ready.
...
Editor stays running between jobs. Combine with
[incremental sync](incremental-sync-protocol) for fastest iteration:
...
- **[Incremental Sync](incremental-sync-protocol)** — Receive workspace updates without full
  clone. Hot runners + incremental sync = fastest possible iteration.
