...
[Retained Workspaces](caching#retained-workspaces) for details.
...
### Structured Build Outputs

Advanced pipelines often produce more than one artifact: the game build, test results, screenshots,
coverage reports, metrics, logs, or exported data. Treat these as typed outputs so post-build hooks
can upload and process each category consistently.

| Output type    | Typical path                  | Common handling                         |
| -------------- | ----------------------------- | --------------------------------------- |
| `build`        | `./Builds/{platform}/`        | Archive and upload as the game artifact |
| `test-results` | `./TestResults/`              | Publish NUnit/JUnit reports             |
| `server-build` | `./Builds/{platform}-server/` | Upload separately from client builds    |
| `coverage`     | `./Coverage/`                 | Publish coverage reports                |
| `metrics`      | `./Metrics/`                  | Store performance and asset statistics  |
| `images`       | `./Captures/`                 | Store screenshots or visual baselines   |
| `logs`         | `./Logs/`                     | Retain structured diagnostic logs       |

Declare output categories in your workflow or project configuration:
...
When a build produces a manifest, keep it beside the uploaded artifacts so downstream jobs can find
outputs by type instead of guessing paths:
...
Structured outputs are still moved by the same storage primitives described on this page. Use hooks
for upload destinations and retention policy, and use the manifest when another job needs to consume
the results.
...
When using [retained workspaces](caching#retained-workspaces), Orchestrator uses distributed locking
to ensure only one build occupies a workspace at a time. Locks are stored in the configured storage
provider:
