...
The standard job — runs the Unity Editor to produce a build artifact. This is what most users
care about.
...
Replace the entire build workflow with your own container steps. Useful for non-Unity workloads
or fully custom pipelines that still benefit from Orchestrator's cloud infrastructure.
...
This is useful when builds exceed GitHub Actions' job time limits, or when you want to
free up your CI runner immediately.
...
- Submodule initialization (if using [submodule profiles](advanced-topics/submodule-profiles))
...
[command hooks](advanced-topics/hooks/command-hooks) to inject steps at specific points in the
build lifecycle.
