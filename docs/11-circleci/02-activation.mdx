...
1. Go to `CircleCI` > `<Organizations Settings>` > `Contexts` > `Create Context` or open an existing
...
3. Remove the `unity/create-activation-file` job from your `.circleci/config.yml` as it's no longer
   necessary.
