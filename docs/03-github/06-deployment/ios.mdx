...
```yaml title=".github/workflows/ios_certificate_setup.yml"
name: iOS Certificate Setup
...
```yaml title=".github/workflows/ios_generate_certs.yml"
name: iOS Generate Certs

on:
  workflow_run:
    workflows: ['iOS Certificate Setup']
    types:
      - completed
  workflow_dispatch:
...
re-run to re-generate certificates. When your certificates expire after a year, you will need to run 
`fastlane match nuke distribution` and re-run the 'iOS Certificate Setup'.

Go to the "Actions" tab in your GitHub repository, and manually run the "iOS Certificate Setup" action
...
> -- **Note:** If `iOS Generate Certs` causes an `Unknown platform` issue, go into the
