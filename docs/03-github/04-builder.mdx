...
> Optionally, the version number can have a `v` prefix, for example `v0.17.2`.  

No configuration required.

##### Tag

Generates the version based on the latest git tag. For example `0.17.2` will generate the version
`0.17.2`. A leading "v" is stripped, so a version of `v1.3.3` will generate a version of `1.3.3`.
**(advanced users)**
