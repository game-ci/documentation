...
Github deploy SSH keys are unique per repository. If you have multiple private repositories in your
packages/manifest.json, you will need to add multiple SSH keys, and use the `webfactory/ssh-agent`
action to manage them and use the right key for each repository.
