...
6. **Return the License**: To free up the seat used during activation, return the license in Unity
   Hub on the local machine you used. Go to:

   - `Unity Hub` > `Preferences` > `Licenses`
   - Select the **Personal license** listed and click **Return license**.
