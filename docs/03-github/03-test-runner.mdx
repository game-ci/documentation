...
If your package has dependencies that are hosted in a private UPM registry, then you will need
to supply a valid authentication token for your registry as an environment variable. Setup the 
[`scopedRegistryUrl`](#scopedRegistryUrl) and [`registryScopes`](#registryScopes) as usual, but
also include a `UPM_REGISTRY_TOKEN` when defining your Unity License. 
...
- uses: game-ci/unity-test-runner@v4
  ...
  with:
    pscopedRegistryUrl: https://example.com/registry
...
packageMode is true. When setting this value, you must also provide [`registryScopes`](#registryScopes)
