...
### 1. Get the LoadBalancer's External-IP address
...
### 2. Set an alias for your server:
...
### 6. Create an artifact storage bucket

```bash
mc mb myminio/artifacts --with-versioning
```

### 7. Grant GameCI account access
...
```bash
mc alias set myminio http://$LOADBALANCER_IP:32000 $ACCESS_KEY $SECRET_KEY
```
