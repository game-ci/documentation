...
You need pdate the package list and upgrade system components prior to installing other software.
...
The following is a curated set of base packages that are dependancies for steps later in the guide
...
3. Import an ssh key
   ...
   - If you have a Github, GitLab, or Launchpad account, you can use `ssh-import-id` to install you
     ssh public-key into your host.
...
5. Craete a password for the user
...
Setting up docker can get trickey because the version available from `apt` is usually out of date.
...
If you used a desktop image instead of a server image you will need to disable your session
managaer.
...
6. Test that it is working by running one of Nvidia's
